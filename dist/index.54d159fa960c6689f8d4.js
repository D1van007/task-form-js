...
;// CONCATENATED MODULE: ./src/utils/mask-phone.js

var addMaskTelInput = function addMaskTelInput() {
  var telInput = document.querySelector("#inputTel");
  inputmask_default()({
    mask: '+375 (99) 999-99-99'
  }).mask(telInput);
};
;// CONCATENATED MODULE: ./src/utils/api.js
var baseURL = "http://localhost:9090/api/registration ";
...
var callModal = function callModal(response) {
  var modalTitleColor = {
    error: "#c41e3a",
    success: "#4BB543",
    "default": "#191919"
...
;// CONCATENATED MODULE: ./src/utils/validation-form.js


var validation_form_form = document.querySelector("#form");
var inputName = validation_form_form.querySelector("#inputName");
var inputEmail = validation_form_form.querySelector("#inputEmail");
var inputTel = validation_form_form.querySelector("#inputTel");
var textareaMessage = validation_form_form.querySelector("#message");
var isValidForm = function isValidForm() {
  var addValidationMessage = function addValidationMessage(isValid, currentInput, message) {
    if (!isValid) {
  ...
  var validName = function validName() {
    var isValidName = !!inputName.value;
    addValidationMessage(isValidName, inputName, "Необходимо ввести имя");
    return isValidName;
  };
  var validEmail = function validEmail() {
    var emailRegex = /^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/;
    var isValidEmail = emailRegex.test(inputEmail.value);
    addValidationMessage(isValidEmail, inputEmail, "Введите корректный адрес эл.почты");
    return isValidEmail;
  };
  var validTel = function validTel() {
    var telRegex = /^\+375 \((17|29|25|33|44)\) \d{3}-\d{2}-\d{2}$/;
    var isValidTel = telRegex.test(inputTel.value);
    addValidationMessage(isValidTel, inputTel, "Введите корректный номер телефона");
    return isValidTel;
  };
  var validMessage = function validMessage() {
    var isValidMessage = !!textareaMessage.value;
...
var validationForm = function validationForm() {
  validation_form_form.addEventListener("submit", function (event) {
    event.preventDefault();
    var formData = new FormData(validation_form_form);
    isValidForm() && submitForm(formData).then(function (res) {
      validation_form_form.reset();
      callModal(res);
    })["catch"](function (error) {
      return callModal(error);
    });
  });
};
